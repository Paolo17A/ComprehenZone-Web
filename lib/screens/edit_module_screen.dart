...
class EditModuleScreen extends ConsumerStatefulWidget {
  ...
  const EditModuleScreen({super.key, required this.moduleID});

  @override
  ConsumerState<EditModuleScreen> createState() => _EditModuleScreenState();
}

class _EditModuleScreenState extends ConsumerState<EditModuleScreen> {
  final titleController = TextEditingController();
  final contentController = TextEditingController();
  final List<dynamic> documentFiles = [];
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      try {
        ...
        titleController.text = moduleData[ModuleFields.title];
        contentController.text = moduleData[ModuleFields.content];

        //  Retrieve and handle additional resources
        List<dynamic> additionalResources =
            moduleData[ModuleFields.additionalResources];
        for (int i = 0; i < additionalResources.length; i++) {
          ...
          fileNameControllers[i].text =
              resourceEntry[AdditionalResourcesFields.fileName];
          downloadLinkControllers.add(TextEditingController());
          downloadLinkControllers[i].text =
              resourceEntry[AdditionalResourcesFields.downloadLink];
        }
        List<dynamic> additionalDocuments =
            moduleData[ModuleFields.additionalDocuments];
        for (int i = 0; i < additionalDocuments.length; i++) {
          documentFiles.add(additionalDocuments[i]
                        ...
                        child: Column(children: [
                          ...
                          all20Pix(
                              child: ElevatedButton(
                                  onPressed: () => editThisModule(context, ref,
  ...
  Widget newLessonHeader() {
    return blackInterBold('EDIT MODULE', fontSize: 40);
  }
