...
class _EditQuizScreenState extends ConsumerState<EditQuizScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      try {
        ...
        _titleController.text = quizData[QuizFields.title];
        _questionController.text =
            quizQuestions[currentQuestion][QuestionFields.question];

        for (int i = 0; i < _choicesControllers.length; i++) {
          _choicesControllers[i].text = quizQuestions[currentQuestion]
              [QuestionFields.options][choiceLetters[i]];
        }
  ...
  Widget newQuizHeader() {
    return blackInterBold('EDIT QUIZ', fontSize: 40);
  }
